...
input('ready to input base_path?');
base_path = uigetdir;
input('ready to input base_path_wav?');
base_path_wav=uigetdir;
if strcmp(alignment,'r')
    input('ready to input base_path_tseries?');
    base_path_tseries=uigetdir;
end
if ~strcmp(cont,'none')
    input('ready to input save_path_pass?')
    save_path_pass = uigetdir;
    input('ready to input save_path_spont?');
    save_path_spont= uigetdir;
end

input('ready to input tot_file_save_path?');
tot_file_save_path=uigetdir;


% base_path =strcat('\\runyan-fs-01\Runyan3\Noelle\Pupil\Noelle Pupil\example data\',mouse,'\',num2str(date),'\pupil_movies'); 
% base_path_wav=strcat('\\runyan-fs-01\Runyan3\Noelle\Pupil\Noelle Pupil\example data\',mouse,'\',num2str(date),'\wavesurfer');
% base_path_tseries=strcat('\\runyan-fs-01\Runyan\Christine\images\',mouse,'\',strcat(date),'\pass\');
% tot_file_save_path =strcat('\\runyan-fs-01\Runyan3\Noelle\Pupil\Noelle Pupil\example data\',mouse,'\',num2str(date),'\processed data'); %this is where final aligned files will be saved, not processed files for individual blocks, those will be saved in the base folder by default
  ...
  %looks for gaps in distribution of object locations - if gaps
   ...
   %Adjust conversion factor according to camera and settings: 
    ...
    %Camera on 2P+:
        ...
        the_areas = (the_radii.^2).*pi;
        the_areas_compare = (the_radii.^2).*pi;
        blink_threshold = 2000;%was 2000 before processing 8/2/2021 tonic dv100 on 9/2/2021  
    end
...
if strcmp('t',alignment)
    %if strcmp('none',cont)
        make_pupil_aligned_tight(base_path,tot_file_save_path,base_path_wav,blocks,pupil_all,mouse,date);
    %else
     %make_pupil_aligned_tight(base_path,tot_file_save_path,base_path_wav,blocks,pupil_all,mouse,date);   
    %end
    
else
    make_pupil_aligned_rough(base_path,tot_file_save_path,base_path_wav,mouse,date,blocks);
end
...
if strcmp('y',dilcon)
    dil_con_events_no_constraints_v2(tot_file_save_path,mouse,date);
end
