...
for bloc=blocks %going through all blocks
block =block+1;
%mouse=num2str(mouse);
...
if bloc<10
    for i=1:length(z)
        if  contains(z(i).name,strcat('_000',num2str(bloc),'.h5'))==1
            hh=h5read(z(i).name,strcat('/sweep_000',num2str(bloc),'/analogScans'));
        end
    end
else
    for i=1:length(z)
        if contains(z(i).name,strcat('_00',num2str(bloc),'.h5'))==1
           hh=h5read(z(i).name,strcat('/sweep_00',num2str(bloc),'/analogScans'));
        end
    end
end
    ...
    for i=2:length(rawFrame)-2
        ...
        m=max(local);
        if rawFrame(i)<10000&&rawFrame(i)==m
...
%alt way of doing it
for b=blocks(2:end)
    if round(max(max(frames{b})),-2)==6600||4000
        x_velocity=cat(2,x_velocity,xv{b}(1:round(max(max(frames{b})),-2)));
        y_velocity=cat(2,y_velocity,yv{b}(1:round(max(max(frames{b})),-2)));
    else
        x_velocity=cat(2,x_velocity,xv{b}(1:max(frames{b})));
