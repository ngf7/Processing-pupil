function [pupil_all,aligned_pupil_unsmoothed, aligned_pupil_smoothed_10, aligned_pupil_smoothed_30, aligned_x_position, aligned_y_position] = make_pupil_aligned_tight(base_path,tot_file_save_path,base_path_wav,blocks,pupil_all,mouse,date)
%function [pupil_all,aligned_pupil_unsmoothed, aligned_pupil_smoothed_10, aligned_pupil_smoothed_30, aligned_x_position, aligned_y_position] = make_pupil_aligned_tight(base_path,tot_file_save_path,base_path_wav,save_path_pass,save_path_spont,blocks,cont,pupil_all,mouse,date)
...
%    end
  ...
  %  if block<10 
   ...
   loadingframes = (framecnt-length(pupil_all{block}.radii.uncut_uncorrected_radii)-1);
   %loadingframes = (framecnt-length(pupil_all{position_block}.radii.uncut_uncorrected_radii));
   rel_first_frame = pupil_all{block}.galvo_on + loadingframes;
   rel_last_frame = pupil_all{block}.galvo_off + loadingframes;
...
%       end
    ...
    if ~(max(RelPupFrame) == length(pupil_all{block}.area.corrected_areas))
...
%     CenterRow = zeros(1,length(RelPupFrame));
    ...
    for i = 1:length(RelPupFrame)
        FrameVal_unsmoothed(i) = pupil_all{block}.area.corrected_areas(RelPupFrame(i));
        FrameVal_smoothed_10(i) = pupil_all{block}.area.smoothed_30_timeframes(RelPupFrame(i));
        FrameVal_smoothed_30(i) = pupil_all{block}.area.smoothed_10_timeframes(RelPupFrame(i));
        CenterCol(i) = pupil_all{block}.center_position.center_column(RelPupFrame(i));
        CenterRow(i) = pupil_all{block}.center_position.center_row(RelPupFrame(i)); 
    end
    ...
    pupil_all{1,block}.alignment.galvo_temp = galvo_temp;
    pupil_all{1,block}.alignment.galvo_peaks = galvo_peak;
    pupil_all{1,block}.alignment.pupil_temp = pupil_temp;
    pupil_all{1,block}.alignment.MatchedFrameInds = RelPupFrame;
    pupil_all{1,block}.alignment.MatchedFrameValsUnsmoothed = FrameVal_unsmoothed;
    pupil_all{1,block}.alignment.MatchedFrameValsSmoothed10 = FrameVal_smoothed_10;
    pupil_all{1,block}.alignment.MatchedFrameValsSmoothed30 = FrameVal_smoothed_30;
...
% if tf_cont==1
%     which_save_path = tot_file_save_path;
% else
%     if tf_which_cont ==1
...
if presence==7
    save(strcat(which_save_path,'\',mouse,'_', num2str(date),'.mat'),'pupil_all','aligned_pupil_unsmoothed','aligned_pupil_smoothed_10','aligned_pupil_smoothed_30','aligned_x_position','aligned_y_position','pup_norm_30','pup_norm_10','pup_norm_unsmoothed','x_velocity','y_velocity','loco_sum','loco_sum_smooth','blockTransitions');
elseif presence==0
    %cd(strcat('\\runyan-fs-01\Runyan\Noelle\reprocessed data\',cont,'\'));
    mkdir which_save_path;
    movefile which_save_path 
    save(strcat(which_save_path,'\',mouse,'_', num2str(date),'_',cont,'.mat'),'pupil_all','aligned_pupil_unsmoothed','aligned_pupil_smoothed_10','aligned_pupil_smoothed_30','aligned_x_position','aligned_y_position','pup_norm_30','pup_norm_10','pup_norm_unsmoothed''x_velocity','y_velocity','loco_sum','loco_sum_smooth','blockTransitions');
end
